...
- **AirspeedVelocity.jl** (MilesCranmer) — actively maintained GH Action that runs a `PkgBenchmark` suite on PR vs. base on the same runner and posts the diff as a PR comment. Works with `benchmark/benchmarks.jl` as-is (no rewrite needed). Worth picking up post-v0.9.0 if/when perf regressions become a real worry. (BenchmarkCI.jl, which would have done the same job, is abandoned — no commits in ~4 years.)
