...
@testset "Spacey.jl" begin
    ...
    d, e, f = rotate_basis_3d([1, 0, 0], [0, 1, 0], [0, 0, 1], π / 3, π / 5, π / 7)
    @test length(Spacey.pointgroup_simple(d, e, f)) == 48
    # hexagonal lattice in unique orientation
    g, h, i = rotate_basis_3d([1, 0, 0], [0.5, √3 / 2, 0], [0, 0, √(8 / 3)], π / 7, π / 11, π / 3)
    @test length(Spacey.pointgroup_simple(g, h, i)) == 24
    # Add a timing check against basic and "fast"; make sure fast is fast
    # hexagonal case
    slow = @belapsed Spacey.pointgroup_simple($g, $h, $i)
    fast = @belapsed Spacey.pointgroup_fast($g, $h, $i)
    println("actual speed up: ", round(slow/fast,digits=2))
    @test isapprox(slow / fast, 27, rtol=Δ)
    # cubic case
    slow = @belapsed Spacey.pointgroup_simple($d, $e, $f)
    fast = @belapsed Spacey.pointgroup_fast($d, $e, $f)
    println("actual speed up: ", round(slow / fast,digits=2))
    @test isapprox(slow / fast, 53, rtol=Δ)
    # tetragonal case
    d, e, f = rotate_basis_3d([1.1, 0, 0], [0, 1, 0], [0, 0, 1], π / 3, π / 5, π / 7)
    slow = @belapsed Spacey.pointgroup_simple($d, $e, $f)
    fast = @belapsed Spacey.pointgroup_fast($d, $e, $f)
    println("actual speed up: ", round(slow / fast,digits=2))
    @test isapprox(slow / fast, 61, rtol=Δ)
    # orthorhombic case
    d, e, f = rotate_basis_3d([1.1, 0, 0], [0, 0.9, 0], [0, 0, 1], π / 3, π / 5, π / 7)
    slow = @belapsed Spacey.pointgroup_simple($d, $e, $f)
    fast = @belapsed Spacey.pointgroup_fast($d, $e, $f)
    println("actual speed up: ", round(slow / fast,digits=2))
    ...
    v = [1, 2, 1]
    w = [2, 1, 1]
    u, v, w = rotate_basis_3d(u, v, w, π / 3, π / 5, π / 7)
