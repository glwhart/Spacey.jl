...
2. **Angle-bound violations:** The Minkowski bounds require angles in [60°, 120°]. When the true angle is exactly 60° or 120° (which happens for hexagonal and rhombohedral lattices), noise can push the computed angle just outside the bound, causing the reduction to perform an unnecessary swap. Both the pre- and post-swap bases are still Minkowski-reduced — they describe the same lattice with different but equally valid choices of short vectors — so the {-1,0,1}³ search space is still complete. The consequence is that the *particular* basis chosen determines how each symmetry operation appears as an integer matrix, which in turn affects the numerical residuals in the downstream integer-matrix test (see below).
...
**Why this isn't where Spacey.jl actually breaks.** `minkReduce`'s loop structure — sort vectors by length, Gauss-reduce in the UV plane, shorten `W` against that plane, repeat until `norm(W) ≥ norm(V)` — is constructed so that its output satisfies all 12 Minkowski conditions by algorithmic invariant. `MinkowskiReduction.jl` exports `isMinkReduced` as a utility but does not call it from `minkReduce` itself; the invariant is enforced by the loop structure, not by a post-check. Noise can cause epsilon-level violations of the conditions, but not the kind of large coefficient mismatch that would make the 27-neighbor search incomplete. For that to happen, the output basis would have to be geometrically non-reduced — e.g., one vector expressible as a ±2 or ±3 combination of the others — which the algorithm's invariants prevent for anything short of catastrophic precision loss (the known ~2²⁵ aspect-ratio failure).

**Where noise actually bites Spacey.jl.** The failure modes that matter live downstream of Minkowski reduction, in the tolerance-driven filtering of candidate operations:

- The integer-matrix test `U = inv(A) * B` requires `U` to be "close to" an integer matrix. How close is controlled by `tol`. A genuinely-lower-symmetry lattice that is geometrically close to a higher-symmetry one produces small-but-nonzero residuals for the operations that *would* close the subgroup to the higher group. Whether those residuals fall below `tol` determines the reported group.
- At a given tolerance, `pointGroup_robust` closes the *largest* subset of accepted candidate operations that forms a group (iterating group sizes 48, 24, 16, …). When enough spurious near-miss operations pass the integer test at the chosen `tol`, they close into a higher-symmetry group than the lattice actually has. The heuristic is principled — the right answer for a noisy *high*-symmetry lattice is often the larger group — but it is biased toward over-promotion for near-miss *low*-symmetry lattices.

**A realistic boundary case: tetragonal near cubic.** Consider `A = diag(1, 1, 1+ε)` for small ε. The true point group is D₄ₕ (16 operations). The cubic group that applies at `ε = 0` adds 32 operations that mix the `c` axis with `a` and `b` — for example, the 120° rotation about the body diagonal [111], which in Cartesian sends `(x, y, z) → (y, z, x)`. Applied to the tetragonal basis this sends `b₁ → (a/c)·b₃`, `b₂ → b₁`, `b₃ → (c/a)·b₂`, so its lattice-coordinate matrix has entries `a/c = 1/(1+ε)` and `c/a = 1+ε` where the cubic case requires exact ±1. The residual on those entries is `O(ε)`. At any `tol ≥ ε`, the 3-fold passes the integer test and the algorithm closes the group at 48 instead of 16. For `ε = 10⁻³` and the default `tol = 10⁻²`, this failure occurs silently with no indication that the answer is tolerance-dependent. This is a real failure — the lattice is tetragonal, Spacey reports cubic — and it is not a Minkowski-reduction problem at all. The existing test suite does not cover it: `test/runtests.jl` exercises "noise on top of a high-symmetry lattice, still find the high-symmetry group," but never "near-miss lower-symmetry lattice, correctly decline to find the higher group."
