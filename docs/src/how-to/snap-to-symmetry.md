...
`LG` is a `Vector{Matrix{Int}}` — exactly the form `snap_to_symmetry_svd` wants. (At the default `tol=0.01`, the same input only finds 8 ops because the 1% distortion now exceeds the tolerance — see [Handle noisy real-world data](handle-noisy-data.md) for how to choose `tol`.)

### 2. Snap

```jldoctest snap
julia> a, b, c, iops, rops = snap_to_symmetry_svd(u, v, w, LG);
```
...
A faster but less robust alternative, `Spacey.snap_to_symmetry_avg`, averages each basis vector independently over its op images. It is reachable as `Spacey.snap_to_symmetry_avg(u, v, w, ops)` but is **not exported** — `snap_to_symmetry_svd` is the recommended path. Use the avg variant only for quick comparisons or when SVD performance becomes a bottleneck (only a problem when you are doing tens of thousands of cases over a large corpus of structures).

## See also

- Reference: [`snap_to_symmetry_svd`](../reference/snap.md)
