...
function parse_poscar_block(block::Vector{String})
    ...
    while i ≤ length(block)
        ...
        # Element is the first token from index 4+ starting with a letter
        # and containing only letters and digits (accepts molecular
        # "atom" labels like H2O, OH, SO4, NH4 used in some hydrated /
        # molecular crystal prototypes).
        elem = nothing
        for p in parts[4:end]
            if !isempty(p) && isletter(p[1]) && all(c -> isletter(c) || isdigit(c), p)
                elem = p; break
            end
        end
        elem === nothing && break
...
# ────────────────────────────────────────────────────────────────────────
function main(input_path::String="/tmp/aflow_raw_clean.txt",
              output_name::String="aflow_structures.jl",
              const_name::String="AFLOW_PART1_STRUCTURES")
    blocks = find_poscar_blocks(input_path)
    ...
    outpath = joinpath(@__DIR__, "..", "test", output_name)
    open(outpath, "w") do f
        println(f, "# Auto-generated by tools/generate_aflow_tests.jl")
        println(f, "# Source: $input_path")
...
if abspath(PROGRAM_FILE) == @__FILE__
    # Usage: julia generate_aflow_tests.jl [input] [output] [const]
    args = ARGS
    if length(args) == 0
        main()
    elseif length(args) == 3
        ...
        error("usage: julia generate_aflow_tests.jl [input_path output_name const_name]")
    end
end
